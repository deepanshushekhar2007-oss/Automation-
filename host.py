...
class Handler(BaseHTTPRequestHandler):
    ...
    # Render health check HEAD request fix
    def do_HEAD(self):
        self.send_response(200)
        self.end_headers()
...
async def run_bot(call: types.CallbackQuery):
    ...
    try:
        ...
        process = await asyncio.create_subprocess_exec(
            "python3",
            "-u",                      # UNBUFFERED OUTPUT (important)
            path,
            stdin=asyncio.subprocess.PIPE,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT   # MERGE LOGS
        )
...
async def read_stream(key, stream):
    ...
    while True:
        ...
        chunk = await stream.read(1)   # read character instead of line
