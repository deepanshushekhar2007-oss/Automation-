...
# ---------------- TELETHON CONNECT & KEEPALIVE ----------------
async def ensure_telethon():
    """Connect Telethon and authorize"""
    while True:
        try:
...
async def keep_telethon_alive():
    """Periodic keepalive + reconnection"""
    while True:
        try:
...
async def main():
    ...
    # Run Telethon in background forever
    asyncio.create_task(tg_client.run_until_disconnected())

    # Keep Telethon alive
    asyncio.create_task(keep_telethon_alive())

    # Start web server to prevent host sleep
    await start_web()

    # Start bot self-ping loop
