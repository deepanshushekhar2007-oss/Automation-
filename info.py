...
async def handle_input(message: Message):
        ...
        try:
            ...
            # -------- /tg --------
                ...
                if not args:
                    ...
                    if not uid:
                        sent = await message.answer(build_msg([], "🆔 TG TO NUMBER"))
                        return
    ...
    try:
        ...
        if mode == "number":
            ...
            if not uid:
                msg = build_msg([], "🆔 TG TO NUMBER")
            elif is_owner(uid, uname):
                msg = (
                    f"<b>😂 Arre Bhai!</b>\n\n"
                    f"👀 Owner ko hi dhoondh rahe ho kya?\n"
                    f"🚫 Ye user search nahi ho sakta.\n"
            ...
            msg = "<pre>❌ Invalid mode!</pre>"

        await message.answer(msg, parse_mode="HTML")
    except Exception as e:
        print(f"PRIVATE {mode} error:", e)
        await message.answer("<pre>❌ Error processing input!</pre>")
