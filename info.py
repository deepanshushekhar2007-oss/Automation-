...
async def cmd_tg(message: Message):
    ...
    try:
        if user_input.isdigit():
            ...
            lookup_id = entity.id
            lookup_uname = getattr(entity, "username", "")
    except Exception as e:
    ...
    # -------- OWNER BLOCK --------
    if lookup_id == OWNER_ID or (lookup_uname and lookup_uname.lstrip("@").lower() == OWNER_USERNAME.lower()):
    ...
    # -------- API CALL --------
    try:
        r = requests.get(TG_API + str(lookup_id), timeout=15)
        data = r.json()
    except Exception as e:
    ...
    if isinstance(result, dict) and result:
        ...
        msg += f"🏷 Country Code - {val(result.get('country_code'))}\n"
    else:
        msg += "❌ No Data Found\n"
...
async def cmd_adhar(message: Message):
    ...
    for res in results_list:
        ...
        msg += f"🏛 State - {val(rc.get('state_name'))}\n"
        msg += f"📍 District - {val(rc.get('district_name'))}\n"
        msg += f"💳 Scheme - {val(rc.get('scheme_name'))}\n"
        msg += f"🆔 RC ID - {val(rc.get('ration_card_no'))}\n"

        members = res.get("members", [])
        msg += "\n👥 Members:\n"
        if members:
            for m in members:
                msg += f"{val(m.get('member_name'))} (UID: {val(m.get('member_id'))})\n"
        else:
...
async def handle_input(message: types.Message):
    ...
    if chat_type == "private":
        ...
        try:
            ...
            if msg:
                await message.answer(msg)  # New UI without copy button
