...
async def cmd_tg(message: Message):
    ...
    # -------- BLOCK OWNER CHECK --------
    blocked_ids = [6860983540]  # Owner ID
    blocked_usernames = ["SPIDYWS"]  # Owner username

    if args.isdigit() and int(args) in blocked_ids:
        await message.answer(
            "<b>😂 Arre Bhai!</b>\n\n"
    ...
    if args.lstrip("@").lower() in [u.lower() for u in blocked_usernames]:
        await message.answer(
            "<b>😂 Arre Bhai!</b>\n\n"
            "👀 Owner ko hi dhoondh rahe ho kya?\n"
            "🚫 Ye user search nahi ho sakta.\n"
...
async def handle_input(message: Message):
    ...
    async def resolve_username(text):
        ...
        Works reliably for new users or changed usernames.
        """
        text = text.strip()
        if text.isdigit():
            return int(text)
        ...
        try:
            ...
            # -------- /tg --------
                ...
                if not args:
                    ...
                    try:
                        r = requests.get(os.getenv("TG_API") + str(user_id_to_lookup), timeout=15)
                        result = r.json().get("result", {})
                    except:
                        sent = await message.answer("❌ API Error!")
                        return
