...
async def handle_input(message: Message):
    ...
    async def resolve_username(text):
        """Return Telegram ID from username or numeric ID."""
        text = text.strip()
        if text.isdigit():
            return int(text)
        ...
        try:
            ...
            # -------- /tg --------
                ...
                if not args:
                    ...
                    user_id_to_lookup = args if args.isdigit() else await resolve_username(args)
                ...
                if not args:
                    ...
                    for res in results_list:
                        ...
                        if members and isinstance(members, list):
                            final_list.extend([m for m in members if isinstance(m, dict)])
                        add_info = res.get("additional_info")
    ...
    try:
        ...
        # -------- tg --------
            ...
            user_id_to_lookup = text if text.isdigit() else await resolve_username(text)
            if not user_id_to_lookup:
                await message.answer("❌ Invalid username or user not found!")
                user_mode.pop(user_id, None)
                return
