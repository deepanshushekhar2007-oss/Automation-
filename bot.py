    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton(
                "📋 List Linked Accounts", callback_data="list_accounts"
            ),
        ],
        [
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "add_account":
        await query.edit_message_text(
            "Please enter your WhatsApp number with country code (e.g., +919876543210):"
        )
        ...
        await query.edit_message_text(
            "Enter minimum delay in seconds (e.g., 250). Minimum allowed is 200s:"
        )
        return WAITING_FOR_DELAY
    elif query.data == "schedule":
        await query.edit_message_text(
            "Enter schedule time in IST (HH:MM) format (e.g., 14:30):"
        )
        return WAITING_FOR_SCHEDULE_TIME
    elif query.data == "start_messaging":
        if len(user_data_store[user_id]["accounts"]) < 2:
            await query.edit_message_text(
                "⚠️ You need at least 2 linked accounts to start messaging.",
        ...
        await query.edit_message_text(
            "✅ Starting messaging between accounts...",
            reply_markup=InlineKeyboardMarkup(
...
async def receive_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id = update.effective_user.id
    await update.message.reply_text(
        f"⏳ Processing number {number}. Opening WhatsApp Web and generating pairing code... This might take a minute."
    )
...
async def get_whatsapp_pairing_code(number, user_id, context):
    try:
        async with async_playwright() as p:
            ...
            await page.wait_for_selector("text=Link with phone number", timeout=60000)
            await page.click("text=Link with phone number")
            ...
            await page.wait_for_selector("[data-testid='pairing-code']", timeout=60000)

            # Extract pairing code
            code_element = await page.query_selector("[data-testid='pairing-code']")
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text=(
                    f"🔑 Your pairing code for {number} is:\n\n{code}\n\n"
            ...
            user_data_store[user_id]["accounts"].append(
                {"number": number, "context": browser_context}  # In real app, save storage_state to disk
            )

            await context.bot.send_message(
                chat_id=user_id, text=f"✅ Account {number} linked successfully!"
            )
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text="❌ Failed to get pairing code or login timed out. Please try again.",
        )
...
async def receive_delay(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if delay < 200:
            await update.message.reply_text(
                "⚠️ Delay should be at least 200 seconds to prevent bans. Setting to 200."
            )
            delay = 200
...
async def messaging_loop(user_id, context):
    ...
    while user_data_store[user_id].get("is_running", False):
        try:
            ...
            await context.bot.send_message(
                chat_id=user_id, text=f"🔄 Starting cycle: Sending {num_msgs} messages."
            )
            ...
            for _ in range(num_msgs):
                ...
                # Placeholder: In full implementation, send via Playwright
                logger.info(
                    f"Simulated: Sending message from {sender['number']} to {receiver['number']}: {msg_text}"
                )
...
async def run_bot():
    scheduler.start()  # ✅ start scheduler inside running loop
